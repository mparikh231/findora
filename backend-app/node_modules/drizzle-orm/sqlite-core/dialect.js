...
import { and, eq, isSQLWrapper } from "../sql/index.js";
...
class SQLiteDialect {
  ...
  buildRelationalQuery({
    ...
    const joins = params ? (() => {
      ...
      return sql.join(
        withEntries.map(([k, join]) => {
          ...
          const joinQuery = isSingle2 ? sql`(select ${json}_object(${jsonColumns}) as ${sql.identifier("r")} from (${innerQuery.sql}) as ${sql.identifier("t")}) as ${sql.identifier(k)}` : sql`coalesce((select ${json}_group_array(json_object(${jsonColumns})) as ${sql.identifier("r")} from (${innerQuery.sql}) as ${sql.identifier("t")}), ${jsonb}_array()) as ${sql.identifier(k)}`;
          return joinQuery;
        }),
        sql`, `
      );
    ...
    const query = sql`select ${selectionSet} from ${getTableAsAliasSQL(table)}${throughJoin}${sql` where ${where}`.if(where)}${sql` order by ${order}`.if(order)}${sql` limit ${limit}`.if(limit !== void 0)}${sql` offset ${offset}`.if(offset !== void 0)}`;
    return {
      sql: query,
      selection
    };
