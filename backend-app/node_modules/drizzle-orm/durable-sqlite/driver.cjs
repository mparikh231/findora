...
function drizzle(client, config = {}) {
  ...
  const session = new import_session.SQLiteDOSession(client, dialect, relations, schema, { logger });
  const db = new DrizzleSqliteDODatabase("sync", dialect, session, relations, schema);
  db.$client = client;
  return db;
}
