...
TFullSchema extends Record<string, unknown> = Record<string, never>, TRelations extends AnyRelations = EmptyRelations, TTablesConfig extends TablesRelationalConfig = TablesRelationalConfig, TSchema extends V1.TablesRelationalConfig = Record<string, never>> {
    ...
    abstract transaction<T>(transaction: (tx: GelTransaction<TQueryResult, TFullSchema, TRelations, TTablesConfig, TSchema>) => Promise<T>): Promise<T>;
}
export declare abstract class GelTransaction<TQueryResult extends GelQueryResultHKT, TFullSchema extends Record<string, unknown> = Record<string, never>, TRelations extends AnyRelations = EmptyRelations, TTablesConfig extends TablesRelationalConfig = TablesRelationalConfig, TSchema extends V1.TablesRelationalConfig = Record<string, never>> extends GelDatabase<TQueryResult, TFullSchema, TRelations, TTablesConfig, TSchema> {
    ...
    static readonly [entityKind]: string;
    constructor(dialect: GelDialect, session: GelSession<any, any, any, any, any>, relations: AnyRelations | undefined, schema: {
        fullSchema: Record<string, unknown>;
        schema: TSchema;
        tableNamesMap: Record<string, string>;
    } | undefined);
