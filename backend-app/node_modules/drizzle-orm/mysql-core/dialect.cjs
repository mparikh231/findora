...
class MySqlDialect {
  ...
  buildRqbColumn(table, column, key) {
    if ((0, import_entity.is)(column, import_column.Column)) {
      const name = import_sql.sql`${table}.${import_sql.sql.identifier(this.casing.getColumnCasing(column))}`;
    ...
    return import_sql.sql`${table}.${(0, import_entity.is)(column, import_sql.SQL.Aliased) ? import_sql.sql.identifier(column.fieldAlias) : (0, import_sql.isSQLWrapper)(column) ? import_sql.sql.identifier(key) : this.nestedSelectionerror()} as ${import_sql.sql.identifier(key)}`;
  }
  ...
  getSelectedTableColumns = (table, columns) => {
    ...
    const columnContainer = table[import_table.Columns];
  ...
  buildColumns = (table, selection, params) => params?.columns ? (() => {
    ...
    return columnIdentifiers.length ? import_sql.sql.join(columnIdentifiers, import_sql.sql`, `) : void 0;
  ...
  buildRelationalQuery({
    ...
    const where = params?.where && relationWhere ? (0, import_expressions.and)(
    ...
    const joins = params ? (() => {
      ...
      return import_sql.sql.join(
        withEntries.map(([k, join]) => {
          ...
          const throughJoin2 = throughTable ? import_sql.sql` inner join ${(0, import_relations.getTableAsAliasSQL)(throughTable)} on ${joinCondition}` : void 0;
          const innerQuery = this.buildRelationalQuery({
            table: targetTable,
            mode: isSingle2 ? "first" : "many",
            schema,
            queryConfig: join,
            tableConfig: schema[tableNamesMap[(0, import_table.getTableUniqueName)(relation.targetTable)]],
            tableNamesMap,
            tables,
          ...
          const joinQuery = import_sql.sql` left join lateral(select ${import_sql.sql`${isSingle2 ? import_sql.sql`json_object(${jsonColumns})` : import_sql.sql`coalesce(json_arrayagg(json_object(${jsonColumns})), json_array())`} as ${import_sql.sql.identifier("r")}`} from (${innerQuery.sql}) as ${import_sql.sql.identifier("t")}) as ${import_sql.sql.identifier(k)} on true`;
          return joinQuery;
        })
      );
    })() : void 0;
    ...
    if (isNested && order) {
      selectionArr.push(import_sql.sql`row_number() over (order by ${order})`);
    }
    const selectionSet = import_sql.sql.join(selectionArr, import_sql.sql`, `);
    const query = import_sql.sql`select ${selectionSet} from ${(0, import_relations.getTableAsAliasSQL)(table)}${throughJoin}${import_sql.sql`${joins}`.if(joins)}${import_sql.sql` where ${where}`.if(where)}${import_sql.sql` order by ${order}`.if(order)}${import_sql.sql` limit ${limit}`.if(limit !== void 0)}${import_sql.sql` offset ${offset}`.if(offset !== void 0)}`;
    return {
      sql: query,
      selection
    };
